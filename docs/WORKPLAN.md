...
| ID | 작업 | 선행 | 크기 | 산출물 | DoD |
|---|---|---|---|---|---|
| **W0** | 저장소·툴체인 셋업 | — | S | `git init`, Next.js 15(App Router)+TS, Tailwind, shadcn/ui 초기화, ESLint·Prettier, Vitest·Playwright 설정, `.env.example`, `.gitignore`(`.env.local` 포함 확인), `pnpm` 스크립트(`dev`/`build`/`typecheck`/`lint`/`test`) | V1 + 루트 페이지가 `pnpm dev`에서 렌더 |
...
| **W2** | DB 스키마·마이그레이션 | U1 | M | `supabase/migrations/0001_init.sql`(PRD §2.3 전체 + `review` 500자 제약), RLS 정책, 분야 프리셋 12종 시드, 진행률 갱신 RPC, `types/database.ts`(생성 타입) | **G1**(적용 전 SQL 리뷰) → 적용 후 V3: 각 테이블 CRUD 왕복 + **타 사용자 토큰으로 접근 시 0행** 확인 |
| **W3** | 인증·세션 | W2 | M | 매직링크 로그인 페이지, `middleware.ts` 세션 갱신, 서버/클라이언트 Supabase 클라이언트 팩토리, 보호 라우트, `auth.users` → `profiles` 자동 생성 트리거 | V1+V2, V4(세션 주입 로그인) → **G2**(사용자 실제 로그인 1회) |
