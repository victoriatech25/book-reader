...
test.describe.serial("도서 등록과 상태 전이", () => {
  ...
  async function registerBook(page: import("@playwright/test").Page, title: string) {
  ...
  test("직접 입력으로 등록하면 상세로 가고 want 상태의 1회독이 생긴다", async ({ page }) => {
  ...
  test("서버 검증에 걸리면 저장하지 않고 사유를 한국어로 보여준다", async ({ page }) => {
    await page.goto("/books/new");
    await page.getByRole("button", { name: "검색 없이 직접 입력하기" }).click();
  ...
  test("읽기 시작 → 잠시 멈춤 → 다시 읽기 → 완독까지 전이한다", async ({ page }) => {
  ...
  test("완독한 책은 기존 기록을 남긴 채 새 회차를 시작한다", async ({ page }) => {
  ...
  test("중단하면 사유가 함께 남는다", async ({ page }) => {
  ...
  test("수정한 내용이 상세에 반영된다", async ({ page }) => {
  ...
  test("등록한 책이 홈 목록에 보이고, 삭제하면 사라진다", async ({ page }) => {
