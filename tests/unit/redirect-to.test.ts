...
describe("authRedirectTo", () => {
  it("두 로그인 방식이 같은 콜백으로 돌아온다", () => {
    expect(authRedirectTo("https://victoria-tech.com", "/")).toBe(
      "https://victoria-tech.com/reader/auth/confirm?next=%2F",
    );
  });

  it("origin의 포트와 스킴을 그대로 지킨다", () => {
    expect(authRedirectTo("http://localhost:3000", "/library")).toBe(
      "http://localhost:3000/reader/auth/confirm?next=%2Flibrary",
    );
  });
  ...
  it("next에 쿼리나 한글이 섞여도 인코딩이 어긋나지 않는다", () => {
    ...
    expect(url.pathname).toBe("/reader/auth/confirm");
